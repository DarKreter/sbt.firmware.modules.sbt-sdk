...
void I2C::Initialize(uint32_t ownAddress)
{
    ...
    if(instance == Instance::I2C_2 && Hardware::uart3.IsInitialized())
        softfault(__FILE__, __LINE__,
                  "Cannot initialize I2C2 along with UART3!");
    {
        ...
        case Instance::NONE:
            softfault(__FILE__, __LINE__,
                      "Somehow instance not set to any I2C...");
        }
    }
...
void I2C::SendMasterIT(uint16_t slaveAddress, uint8_t* data, size_t numOfBytes)
{
    ...
    if(state.txRxState)
        ...
        if((xEventGroupGetBits(state.txRxState) & Hardware::txBit) == 0) {
            ...
            HAL_I2C_Master_Transmit_IT(&state.handle, slaveAddress, data,
                                       numOfBytes);
...
{
    HAL_I2C_Master_Transmit(&state.handle, slaveAddress, data, numOfBytes,
                            timeout);
}
                          ...
                          size_t numOfBytes)
{
    ...
    if(state.txRxState)
        ...
        if((xEventGroupGetBits(state.txRxState) & Hardware::rxBit) == 0) {
            // If I2C is not busy, transmit and set TX flag to busy
            HAL_I2C_Master_Receive_IT(&state.handle, slaveAddress, data,
                                      numOfBytes);
...
void I2C::ReceiveMasterRCC(uint16_t slaveAddress, uint8_t* data,
                           size_t numOfBytes)
{
    HAL_I2C_Master_Receive(&state.handle, slaveAddress, data, numOfBytes,
                           timeout);
}
                         ...
                         uint8_t registerSize, uint8_t* data, uint16_t dataSize)
{
    ...
    if(state.txRxState)
        ...
        if((xEventGroupGetBits(state.txRxState) & Hardware::rxBit) == 0) {
            // If I2C is not busy, transmit and set TX flag to busy
            HAL_I2C_Mem_Read_IT(&state.handle, slaveAddress, registerAddress,
...
void I2C::ReadRegisterRCC(uint16_t slaveAddress, uint16_t registerAddress,
                          ...
                          uint16_t dataSize)
{
    HAL_I2C_Mem_Read(&state.handle, slaveAddress, registerAddress, registerSize,
                     data, dataSize, timeout);
}
                          ...
                          uint16_t dataSize)
{
    ...
    if(state.txRxState)
        ...
        if((xEventGroupGetBits(state.txRxState) & Hardware::txBit) == 0) {
            // If I2C is not busy, transmit and set TX flag to busy
            HAL_I2C_Mem_Write_IT(&state.handle, slaveAddress, registerAddress,
                                 registerSize, data, dataSize);
            xEventGroupSetBits(state.txRxState, Hardware::txBit);
        }
}
...
void I2C::WriteRegisterRCC(uint16_t slaveAddress, uint16_t registerAddress,
                           ...
                           uint16_t dataSize)
{
    HAL_I2C_Mem_Write(&state.handle, slaveAddress, registerAddress,
