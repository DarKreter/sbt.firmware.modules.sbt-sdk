...
void SPI_t::Initialize()
{
...

    {
        ...
        case Instance::NONE:
            softfault(__FILE__, __LINE__,
                      "Somehow instance not set to any SPI...");
        }
    }
    ...
    if(mode == OperatingMode::INTERRUPTS) {
        ...
        //        if((deviceType == DeviceType::MASTER && mosiEnabled) ||
        //        (deviceType == DeviceType::SLAVE && misoEnabled))
        //            __HAL_SPI_ENABLE_IT(&handle, SPI_IT_TXE);
        //        if((deviceType == DeviceType::MASTER && misoEnabled) ||
        //        (deviceType == DeviceType::SLAVE && mosiEnabled))
...
void SPI_t::SetBaudRate(int32_t _baudRate)
{
    ...
    if(_baudRate < 1 || 18'000'000 < _baudRate)
        softfault(__FILE__, __LINE__,
                  "Baud rate for SPI must be between 1 and 18'000'000");
